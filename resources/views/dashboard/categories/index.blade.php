            ...
            <div class="col-md-6">
                <div class="table-tool-part mb-3">
                    <a class="btn  btn-outline-primary"
                        href="{{ route('dashboard.categories.create') }}">{{ trans('Add') }}</a>
                    <a class="btn  btn-outline-success"
                        href="{{ route('dashboard.categories.trash') }}">{{ trans('Trash') }}</a>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                @can("categories.delete")
                                    <form method="post"
                                        action="{{ route('dashboard.categories.destroy', ['id' => $category->id]) }}">
                                        @csrf
                                        @method('delete')
